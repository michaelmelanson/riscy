...
use riscy_isa::{
    AmoFunction, AmoWidth, BranchOperation, DecodingStream, EnvironmentFunction, Instruction,
    LoadWidth, MiscMemFunction, Op32Function, OpFunction, OpImm32Function, OpImmFunction, Opcode,
    Register, StoreWidth, SystemFunction,
};
...
impl<S: Subsystem> RiscvMachine<S> {
    pub fn new(memory: Memory, entry: u64) -> Self {
        ...
        csr.set(CSRRegister::MachineISA, 2 << (64 - 2));
        csr.set(CSRRegister::MachineStatus, (2 << 32) | (2 << 34));
        ...
        match instruction {
                ...
                match opcode {
                    Opcode::Op(function) => {
                        ...
                        let result = match function {
                            ...
                            OpFunction::MULH => {
                                ((lhs as i64 as i128).overflowing_mul(rhs as i64 as i128).0 >> 64)
                                    as u64
                            }
                            OpFunction::MULHU => {
                                ((lhs as u128).overflowing_mul(rhs as u128).0 >> 64) as u64
                            }
                            OpFunction::DIV => {
                                if rhs == 0 {
                                    -1i64 as u64
                                } else {
                                    (lhs as i64).overflowing_div(rhs as i64).0 as u64
                                }
                            }
                            OpFunction::DIVU => {
                                if rhs == 0 {
                            ...
                            OpFunction::REM => {
                                if rhs == 0 {
                                    ...
                                    (lhs as i64).overflowing_rem(rhs as i64).0 as u64
                                }
                            }
                            OpFunction::REMU => {
                                if rhs == 0 {
                                    ...
                                    (lhs as u64).overflowing_rem(rhs as u64).0
                                }
                            }
                        ...
                        let result = match function {
                            ...
                            Op32Function::DIVW => {
                                if rhs == 0 {
                                    ...
                                    (lhs as i32).overflowing_div(rhs as i32).0 as u32
                                }
                            }
                            Op32Function::DIVUW => {
                                if rhs == 0 {
                                    ...
                                    lhs.overflowing_div(rhs).0 as u32
                                }
                            }
                            ...
                            Op32Function::REMW => {
                                if rhs == 0 {
                                    ...
                                    (lhs as i32).overflowing_rem(rhs as i32).0 as u32
                                }
                            }
                            Op32Function::REMUW => {
                                if rhs == 0 {
            ...
            Instruction::I {
                ...
                Opcode::JAlr => {
                    ...
                    log::debug!(
                        "{:#016x}: JAlr jumping to {:#08x} + {} = {:#08x} with link {:#08x}",
                        pc,
                        base,
                        offset,
                        target,
                ...
                Opcode::OpImm32(function) => {
                    ...
                    log::debug!(
                        "OP-IMM32: {:#08x} ({}) {:?} {:#08x} ({}, shamount={}) = {:#016x} ({})",
                        lhs,
                        lhs as i64,
                        function,
                        rhs,
            ...
            Instruction::IS {
                ...
                Opcode::System(function) => match function {
                    SystemFunction::Environment(function) => match function {
                        ...
                        EnvironmentFunction::MRET => {
                            let mepc = self
                                .state()
                                .csr
                                .get(CSRRegister::MachineExceptionProgramCounter);
                    ...
                    SystemFunction::CSR(function) => {
                        ...
                        state.csr.execute(
                            function,
                            CSRRegister::from_u32(imm as u32),
                            rs1,
                            rd,
                            &mut state.registers,
                        );
                ...
                Opcode::Store(width) => {
                    ...
                    match width {
                        StoreWidth::DoubleWord => {
                            self.memory.store_double_word(effective_address, value)
                        }
                        StoreWidth::Word => self.memory.store_word(effective_address, value),
                        StoreWidth::HalfWord => {
                            self.memory.store_halfword(effective_address, value)
                        }
            ...
            Instruction::B {
                ...
                Opcode::Branch(operation) => {
                    ...
                    if matches {
                        ...
                        log::debug!(
                            "{:#016x}: Branch condition ({:#08x} {:?} {:#08x}) did not match",
                            pc,
                            lhs,
                            operation,
                            rhs
            ...
            Instruction::U { imm, rd, opcode } => match opcode {
                Opcode::AuiPc => {
                    ...
                    log::debug!(
                        "{:#016x}: AUIPC computed value {:#016x} = {:#016x} + {:#016x} ({})",
                        pc,
                        value,
                        pc,
                        offset,
                ...
                Opcode::Lui => {
                    ...
                    let value = imm;
                    log::debug!(
                        "Lui: prior={:#016x}, imm={:#016x}, value={:#016x}",
                        prior,
                        imm,
                        value
                    );
            ...
            Instruction::J { imm, rd, opcode } => match opcode {
                Opcode::JAl => {
                    ...
                    log::debug!(
                        "{:#016x}: Jumping to {:#016x} + {} = {:#016x} with link {:#016x}",
                        pc,
                        pc,
                        offset,
                        target,
            ...
            Instruction::CIW { opcode, imm, rd } => match opcode {
                Opcode::CADDI4SPN => {
                    ...
                    log::debug!(
                        "{:#016x}: C.ADDI4SPN added stack pointer {:#016x} + {} = {:#016x}",
                        pc,
                        source,
                        imm,
                        result
            ...
            Instruction::CI { opcode, rd, imm } => match opcode {
                Opcode::CADDI16SP => {
                    ...
                    log::debug!(
                        "{:#016x}: C.ADDI16SP added stack pointer {:#016x} + {} = {:#016x}",
                        pc,
                        source,
                        imm,
                        result
                    ...
                    self.state_mut()
                        .registers
                        .set(Register::StackPointer, result);
                }
                ...
                Opcode::CSLLI => {
                    ...
                    log::debug!(
                        "{:#016x}: C.SLLI computed {:#016x} << {:#016x} = {:#016x}",
                        pc,
                        lhs,
                        shamt,
                        value
            ...
            Instruction::CB { opcode, rs1, imm } => match opcode {
                ...
                Opcode::CBEQZ | Opcode::CBNEZ => {
                    ...
                    if matches {
                        ...
                        log::debug!(
                            "{:#016x}: {:?} condition does not match (value={:#016x})",
                            pc,
                            opcode,
                            value
                        );
            ...
            Instruction::CA { opcode, rs2, rd } => {
                ...
                let result = match opcode {
                    ...
                    Opcode::CADDW => {
                        (lhs as u32 as i32).wrapping_add(rhs as u32 as i32) as i64 as u64
                    }
                    Opcode::CSUBW => {
                        (lhs as u32 as i32).wrapping_sub(rhs as u32 as i32) as i64 as u64
                    }
            ...
            Instruction::CR { opcode, rs1, rs2 } => {
                match opcode {
                    ...
                    Opcode::CMV => {
                        ...
                        log::debug!(
                            "{:#016x}: C.MV copying {:#016x} from {:?} into {:?}",
                            pc,
                            value,
                            rs2,
                            rs1
                    ...
                    Opcode::CJALR => {
                        ...
                        log::debug!(
                            "{:#016x}: C.JALR jumping to {:#016x} with link {:#016x}",
                            pc,
                            address,
                            link
                        );
                        self.state_mut()
                            .registers
                            .set(Register::ReturnAddress, link);
                        next_instruction = address;
                    }
            ...
            Instruction::CSS { opcode, imm, rs2 } => {
                ...
                log::debug!(
                    "{:#016x}: {:?} writing {:#016x} to {:#016x} (SP) + {:#016x} ({}) = {:#016x}",
                    pc,
                    opcode,
                    value,
                    base,
...
mod tests {
    ...
    fn test_store_double_word() {
        ...
        machine
            .memory
            .store_double_word(0x1000, 0x1234567890abcdefu64)
            .unwrap();

        assert_eq!(
            machine.memory.slice(0x1000).expect("slice")[0..=0x7],
    ...
    fn test_load_double_word() {
        ...
        machine
            .memory
            .store_byte(0x1007, 0x12)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1006, 0x34)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1005, 0x56)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1004, 0x78)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1003, 0x90)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1002, 0xab)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1001, 0xcd)
            .expect("load failed");
        machine
            .memory
            .store_byte(0x1000, 0xef)
            .expect("load failed");

        let actual = machine
            .memory
            .load_double_word(0x1000)
            .expect("load failed");
