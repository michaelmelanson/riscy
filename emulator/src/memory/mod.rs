...
impl Memory {
  ...
  pub fn store_double_word(&mut self, address: u64, value: u64) -> MemoryResult<()> {
    log::trace!("Writing double word {:#016x} ({}) to memory addresses {:#016x}-{:#016x}", value, value, address, address + 7);
  ...
  pub fn store_word(&mut self, address: u64, value: u64) -> MemoryResult<()> {
    log::trace!("Writing word {:#08x} ({}) to memory address {:#016x}", value, value, address);
  ...
  pub fn store_halfword(&mut self, address: u64, value: u64) -> MemoryResult<()> {
    log::trace!("Writing half-word {:#04x} ({}) to memory address {:#016x}", value, value, address);
  ...
  pub fn store_byte(&mut self, address: u64, value: u64) -> MemoryResult<()> {
    log::trace!("Writing byte {:#02x} ({}) to memory address {:#016x}", value, value, address);
    let region = self.find_region_mut(address)?;
    let offset = address - region.base();
  ...
  pub fn load_double_word(&self, address: u64) -> MemoryResult<u64> {
    ...
    let offset = address - region.base();
    let value = 
      (region.read(offset + 7)? as u64) << 56 |
    ...
    log::trace!("Loaded {:#016x} ({}) from memory address {:#016x}", value, value, address);
  ...
  pub fn load_word(&self, address: u64) -> MemoryResult<u64> {
    ...
    let offset = address - region.base();
    let value = 
      (region.read(offset + 3)? as u32) << 24 |
      (region.read(offset + 2)? as u32) << 16 |
      (region.read(offset + 1)? as u32) << 8 |
      (region.read(offset + 0)? as u32) << 0;

    log::trace!("Loaded word {:#08x} ({}) from memory address {:#016x}", value, value, address);
  ...
  pub fn load_halfword(&self, address: u64) -> MemoryResult<u64> {
    ...
    let offset = address - region.base();
    let value = 
      (region.read(offset + 1)? as u16) << 8 |
      (region.read(offset + 0)? as u16) << 0;

    let value = value as i16 as u64;

    log::trace!("Loaded half-word {:#04x} ({}) from memory address {:#016x}", value, value, address);
  ...
  pub fn load_halfword_unsigned(&self, address: u64) -> MemoryResult<u64> {
    ...
    log::trace!("Loaded unsigned half-word {:#04x} ({}) from memory address {:#016x}", value, value, address);
  ...
  pub fn load_byte(&self, address: u64) -> MemoryResult<u64> {
    ...
    log::trace!("Loaded unsigned byte {:#02x} ({}) from memory address {:#016x}", value, value, address);
