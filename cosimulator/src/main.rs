...
struct SpikeSimulator(
    Sender<SpikeSimulatorCommand>,
    Receiver<SpikeSimulatorEvent>,
    JoinHandle<()>,
);
...
impl SpikeSimulator {
    fn new(path: &str) -> Self {
        ...
        let handle = std::thread::spawn(move || {
            ...
            loop {
                ...
                match command {
                    SpikeSimulatorCommand::Send(s) => {
                        writeln!(stdin, "{}", s).expect("write to stdin")
                    }
                };
                ...
                event_sender
                    .send(SpikeSimulatorEvent::Output(response.to_string()))
    ...
    fn send_command(&mut self, command: &str) -> String {
        debug!("Sending to Spike: {}", command);

        self.0
            .send(SpikeSimulatorCommand::Send(command.to_string()))
            .expect("sending command");
        let response = self.1.recv().expect("receiving response");

        debug!("Received from Spike: {:?}", response);
        match response {
...
impl Simulator for SpikeSimulator {
    fn state(&mut self, memory_addresses: &[u64]) -> SimulatorState {
        ...
        if prefix != "0x" {
            panic!(
                "unexpected prefix {:?} on PC response: {:?}",
                prefix, pc_string
            );
        }
        ...
        for index in 0..32 {
            ...
            let value = u64::from_str_radix(&value[2..], 16)
                .expect(&format!("parsing {:?} for register {:?}", value, register));
        ...
        for address in memory_addresses {
            ...
            let value = u64::from_str_radix(&value[2..], 16).expect(&format!(
                "parsing {:?} for memory address {:?}",
                value, address
            ));
            memory.push((*address, value));
        }
...
struct Cosimulator<Experiment: Simulator, Control: Simulator> {
