...
impl Instruction {
  pub fn from_16bits(encoded: u16) -> Instruction {
    ...
    match opcode {
      ...
      Opcode::CADDI16SP => {
        ...
        let imm = ((if sign_bit { 0b1111111111111111 << 10 } else { 0 })
                     | nzuimm_9 | nzuimm_4 | nzuimm_6 | nzuimm_8_7 | nzuimm_5) as i16 as i64;
      ...
      Opcode::CSRLI | Opcode::CSRAI | Opcode::CANDI => {
        ...
        let imm = 
          (if sign_bit { (-1i16 as u16) << 6 } else { 0 }) |
          shamt_5 | shamt_4_0;
        let imm = imm as i16;
      ...
      Opcode::CBEQZ | Opcode::CBNEZ => {
        ...
        let imm = 
          (if sign_bit { (-1i16 as u16) << 9 } else { 0 }) |
  ...
  pub fn from_32bits(encoded: u32) -> Instruction {
    ...
    let instruction = match opcode {
      ...
      Opcode::JAlr | Opcode::Load(_) | Opcode::OpImm(_) | Opcode::OpImm32(_) | Opcode::MiscMem(_) => {
        let imm11 = ((encoded >> 31) & 0b1) << 11;
        let imm10_0 = (encoded >> 20) & 0b111111111111;
        let sign_bit = imm11 != 0;

        let imm: u32 = (if sign_bit { (-1i32 as u32) << 12 } else { 0 })
                     | imm11 | imm10_0;
      ...
      Opcode::Store(_) => {
        ...
        let imm = (if sign_bit { (-1i32 as u32) << 12 } else { 0 }) 
                     | imm11 | imm10_5 | imm4_0;
